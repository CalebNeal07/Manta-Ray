...
package com.koibots.robot.util;
...
public class SparkUtils {
    ...
    private static void configure(CANSparkBase spark, Supplier<REVLibError> config, int attempt) {
        ...
        if (attempt >= 1) {
            DriverStation.reportWarning(
                    name(spark) + ": setting parameter failed: " + attempt + "/" + MAX_ATTEMPTS,
                    false);
        }
     ...
     * @see Sensor
     * @see Data
     * @see <a href="https://docs.revrobotics.com/brushless/spark-max/control-interfaces">...</a>
     */
